...
Easy Attributes is a Ruby DSL to give more control to attributes. It provides a unique attribute enum setup,
and conversions to bytes and float-as-integer (money, frequencies, Ratings, etc.) 
/ fixed-decimal precision as integer (See the easy_money gem).

It's humble, simple, just meant to work.

Tested with ruby 1.9

Woops! The gem version 0.2 bring a new interface and API. if you have issues, remember your version the gem in your Gemfile (~0.1.3)
...
Declaration:
 ...
 class Record < ObjectRelationalMapperOfChoice
   ...
   attr_values :http_status, name:{value:404, name:"Lost", title:"Not Found"}
   attr_sequence  :field, :name, :name, ... ,:start=>1, :step=>1
   attr_shared, :field, field2: :shared_field
   attr_money  :amount, :units=>"dollars", :unit=>'$', :negative=>'%.2f CR'
   attr_fixed  :rating
   attr_bytes  :bandwidth
 end

== attr_values :field, name:value, name:{value:value, title:text, name:word}, ...
...
Will define:
  ...
  field_is?(:greater_than, :name) # true if the value is greater than the value of :name
    ...
    :greater_than, :gt, :greater_<Up>than_or_equal, :ge, :less_than, :lt, :less_than_or_equal, :le
  field_is?(:between, :name1, :name2) # true if the value is between the corresponding values
  MyClass.field_of(:name) #=> value
  MyClass.options      #=> [["Option Name", :symbol], ...] ## Suitable for HTML <option> list
...
== attr_enum :field, :zero, :one, 8, :nine, ..., start:0, step:1
...
This is a synonym for attr_money, where it represents a float-as-integer 
(or fixed-decimal precision as integer) formatting.
...
Copyright (c) 2012 Allen Fair. See LICENSE for details.
